...
Mounted only in the authenticated app shell (`src/components/genius/`). Never on marketing, docs, auth, or public status pages.

| Goal | When | Allowed metadata |
| --- | --- | --- |
| `genius_widget_ready` | Genius script finished loading | none |
| `genius_opened` | User opened the widget from a Fajita trigger | `source` (`floating`, `command_palette`, `account_menu`, `inline`, `button`, `support_page`), optional `category` |
